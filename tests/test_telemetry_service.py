"""Unit tests for app/services/telemetry_service.py — the by-host/by-model
group_by() counts are plain SQL (same convention as
tests/test_stats_service.py), but the latency time-series bucketing and
p95 figure are hand-rolled Python (see that module's own docstring on
why SQL isn't portable enough here), so those get direct, exact-value
coverage rather than just a smoke test."""
...
async def test_summary_on_an_empty_database(db):
    summary = await telemetry_service.get_ollama_telemetry_summary(db)
