...
def test_build_env_omits_unset_fields(monkeypatch):
    ...
    assert env["OLLAMA_MODELS"] == ollama_process.auto_detect_models_path()


def test_build_env_honors_a_models_path_override():
    env = ollama_process._build_env(_config(models_path="/mnt/big-disk/ollama-models"))
    assert env["OLLAMA_MODELS"] == "/mnt/big-disk/ollama-models"


def test_auto_detect_models_path_is_pairings_own_models_folder():
    assert ollama_process.auto_detect_models_path() == str(ollama_process.BASE_DIR / "models")
