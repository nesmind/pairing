...
async def test_available_versions_falls_back_to_the_pinned_repo_when_none_is_given(monkeypatch):
    ...
    async def fake_list_tags(repo):
        seen["repo"] = repo
        return ["v0.35.1", "v0.35.0"]

    monkeypatch.setattr(github_releases, "list_tags", fake_list_tags)

    result = await comfyui_admin.available_versions(repo=None, _admin=None)

    assert seen["repo"] == COMFYUI_GITHUB_REPO
    assert result.versions == ["v0.35.1", "v0.35.0"]
...
async def test_available_versions_uses_a_given_repo_override(monkeypatch):
    ...
    await comfyui_admin.available_versions(repo="me/comfy-fork", _admin=None)

    assert seen["repo"] == "me/comfy-fork"
