...
async def test_retrieve_context_channel_scope_excludes_private_docs(db, user, monkeypatch):
    ...
    async def fake_embed(_query, _model):
        return [1.0, 0.0]

    async def fake_get_default_embedding_model(_db):
        return "fake-embed-model"

    monkeypatch.setattr(document_retrieval, "embed", fake_embed)
    monkeypatch.setattr(
        document_retrieval.model_catalog_service, "get_default_embedding_model", fake_get_default_embedding_model
    )
