...
from app.services.ollama_client import OllamaError, chat_stream, stop_model
                ...
                async with asyncio.timeout(effective_timeout):
                    async for chunk in chat_stream(model, ollama_messages, params):
                        ...
                        if len(unflushed) >= _FLUSH_CHARS or (now - last_flush) >= _FLUSH_INTERVAL_SECONDS:
                            ...
                            unflushed = ""
                            last_flush = now
                            # Cross-instance guard (see reply_cross_instance_service's own docstring): a
                            # delete_message call served by a *different* sibling instance must stop this task
                            # before it blindly overwrites that deletion once done.
                            fresh = await reply_cross_instance_service.refresh_or_none(db, message)
