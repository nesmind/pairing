...
async def create_conversation(db: AsyncSession, body: ConversationCreate, user: User) -> Conversation:
    ...
    An explicit `body.model` is validated the same strict way update_conversation's own model switch is (a
    caller asking for something specific that isn't installed is a clear 400, not a silent substitution). The
    *default* case is different: get_default_model is a raw stored preference with no installed-check of its
    own, so it routinely still names a model uninstalled since it was last set (confirmed live: an admin
    switching the active engine, or simply deleting a model, both leave this stale) — resolve_installed_model
    falls back gracefully there instead of handing a brand-new chat a model that will 404 the instant a message
    is sent."""
...
async def delete_conversation(db: AsyncSession, conversation: Conversation) -> None:
    ...
    Any reply still streaming in this conversation is cancelled up front,
    same mark_message_deleted-before-cancel_generation ordering
    _cancel_and_clear uses and for the same reason (no `await` between the
    two, else reply_generation_service's own write for this reply can race
    the cascade-delete below and win) — without this, the still-running
    generation only notices the row is gone via
    reply_cross_instance_service's watchdog, several seconds later, and
    even then only within reply_termination_service's own cleanup path."""
    if conversation.channel_id is not None:
        raise HTTPException(
            status_code=400,
