...
from app.config import EMBEDDING_MODEL, EMBEDDING_NUM_CTX
...
async def has_embedding_model() -> bool:
    """Whether the embedding model RAG depends on is actually available. Used to gray out the RAG toggle instead of
    letting it fail at chat time with a confusing error."""
    try:
        models = await list_models()
    except OllamaError:
        return False
    return any(m.get("name", "").startswith(EMBEDDING_MODEL) for m in models)
...
async def embed(text: str) -> list[float]:
    """Turns a piece of text into an embedding vector using EMBEDDING_MODEL. Used both when ingesting documents
    (app/rag.py) and when embedding the user's live question to find matching chunks.
