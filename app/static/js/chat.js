 ...
 * `errorText`, only meaningful with status "error", overrides the
 * generic REPLY_FAILED_NOTICE — used by sendMessage's catch handler for
 * a request that never actually started generating (e.g. rejected with
 * 409 because another member's reply was already in progress), where the
 * server's own specific reason is more useful than the generic notice. */
...
function addMessageBubble(
  ...
  renderReplyBody(bubble, content, status, sources);
...
async function runChannelPoll(conversationId) {
  ...
  for (const message of result.messages) {
    ...
    renderReplyBody(bubble, message.content, message.status, message.sources);
...
async function openModelPicker() {
  ...
  for (const model of models) {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "w-full truncate rounded-md px-2 py-1.5 text-start text-sm transition-colors " + (
...
async function readAssistantReplyStream(
  ...
  for await (const newText of chunks) {
    ...
    for (const frame of frames) {
      ...
      if (payload.error) {
        // Rendered the same way a reload of this conversation would
        // show it (see renderReplyBody) — whatever text had already
        // streamed stays visible, with the standard failure notice
        // below it, rather than replacing the bubble with a one-off
        // plain-text error message that reload could never reproduce.
        // Shouldn't happen server-side when askAi is false (no reply is
        // ever attempted — see build_reply_stream), but fail safe rather
        // than silently swallow it if it somehow did.
        if (bubble) {
          renderReplyBody(bubble, fullText, "error");
          return;
        }
