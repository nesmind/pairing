...
async function pullModel(entry, button, progressEl, onDone = refreshCatalogs) {
...
function renderModelRow(entry) {
  ...
  if (!entry.hardware_ok) {
    ...
    reason.textContent = entry.unavailable_reason
      || `Requires ~${entry.min_ram_gb}GB RAM/VRAM — this machine has ~${currentCatalog.hardware.total_gb}GB.`;
...
function renderCatalogGroup(containerEl, entries) {
  ...
  for (const vendor of vendors) {
    ...
    for (const family of families) {
      ...
      heading.innerHTML = `${escapeHtml(family)} <span class="font-normal text-slate-500">— ${escapeHtml(vendor)}</span>`;
...
function renderHfRepoFiles(repo) {
  ...
  for (const file of sortedFiles) {
    ...
    label.innerHTML = `${escapeHtml(`${file.filename} (${formatSize(file.download_gb)})`)}${recommendedBadge}`;
    ...
    addBtn.addEventListener("click", async () => {
      ...
      hfSearchStatusEl.textContent = "Adding…";
      hfSearchStatusEl.classList.remove("text-red-400");
      try {
        await api("/api/settings/model-catalog/extended", {
          method: "POST",
          body: JSON.stringify({ repo_id: repo.repo_id, filename: file.filename }),
        });
        hfSearchStatusEl.textContent = `Added "${file.filename}".`;
        hideHfRepoFiles();
        hfSearchResultsEl.classList.add("hidden");
        await loadExtendedCatalog();
        renderExtendedModelCatalog();
...
function renderEmbeddingCatalog() {
  ...
  for (const entry of currentEmbeddingCatalog.entries) {
    ...
    const defaultBadge = isDefault
      ...
      : "";
    label.innerHTML =
      `<span class="block truncate font-medium text-slate-100">${escapeHtml(modelName || entry.tag)}${defaultBadge}</span>` +
      `<span class="block truncate text-xs text-slate-500">${escapeHtml(metaParts.join(" · "))}</span>`;
...
async function loadTarget(targetId) {
  ...
  try {
    ...
    // Nothing to narrow against — num_ctx stays at its default max.
  }
...
const EXTERNAL_SERVERS = ["ollama", "comfyui"];
...
function addRemoteHostRow(section, value = "") {
  ...
  input.type = "text";
  input.placeholder = "http://10.0.0.5:11434";
  input.value = value;
  input.className =
...
async function loadExternalServersIntoEditor() {
  await Promise.all(EXTERNAL_SERVERS.map(loadServerSection));
...
for (const server of EXTERNAL_SERVERS) {
  ...
  section.querySelector('[data-action="save"]').addEventListener("click", async () => {
    ...
    statusEl.textContent = "Saving…";
    try {
      // Ollama's local-mode parameters can't apply live (Ollama only
      // reads them at its own process startup) — if it's already
      // running, saving must restart it, so this confirms first, same
      // as the old "Ollama concurrency" save button already did.
      if (server === "ollama" && body.mode === "local" && (await api("/api/settings/ollama/status")).running) {
        const confirmed = confirm(
          "Saving restarts Ollama — every reply currently being generated, on every local " +
        ...
        statusEl.textContent = "Restarting Ollama…";
        renderServerStatus(section, await api("/api/settings/ollama/apply", { method: "POST", body: JSON.stringify(body) }));
        statusEl.textContent = "Saved — Ollama restarted.";
        return;
      }
  ...
  for (const action of ["start", "stop"]) {
    section.querySelector(`[data-action="${action}"]`).addEventListener("click", async () => {
      ...
      try {
        ...
        // stuck/broken status rather than what it actually was.
        if (server === "ollama" && action === "start" && result.running) {
          try {
  ...
  const installBtn = section.querySelector('[data-action="install"]');
  if (installBtn) installBtn.addEventListener("click", () => installServer(server, section));
}
 ...
 */
async function installServer(server, section) {
  ...
  try {
    ...
    // clicking Start) happened to trigger a reload some other way.
