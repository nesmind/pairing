 ...
 * app/services/reply_generation_service.py, app/services/ollama_client.py,
 * app/services/ollama_pool.py, app/services/reply_broadcast_service.py,
 ...
 * or "Ollama" (the separate model-serving process this app talks to) —
...
const WORKFLOW_NODES = [
  ...
  },
  {
    ...
    detail: {
      ...
      body: "Creates and commits the user's own Message row immediately (durably saved regardless of whether generation later succeeds). In \"simple\" title mode, sets the conversation's title here too — pure local text processing, no model call. Yields {user_message_id} as the very first SSE event so the browser can tag its optimistic bubble. Builds a trimmed conversation history (kept under a token-budget proxy based on num_ctx), then hands off to system-prompt assembly.",
    },
  },
  {
  ...
  {
    id: "ollama_client_stream",
    label: "ollama_client.chat_stream()",
    category: "service",
    x: 560,
    y: 810,
    w: 220,
    ...
    detail: {
      server: APP_NAME,
      title: "Resolving a host and calling Ollama",
      location: "app/services/ollama_client.py — chat_stream",
      body: "This is the step that actually hands the conversation off to the AI language model for inference. Picks which Ollama host to use via ollama_pool — a cached, in-memory host list (local mode: the one Ollama process this app manages itself; remote mode: the admin's configured host list), with automatic failover to a different host if the very first connection attempt fails (never mid-stream, which would risk duplicating output already sent). Issues the real POST {host}/api/chat with stream=True. If the model is a \"thinking\" model, raw reasoning tokens are stripped before anything is yielded upward — this filtering is unconditional, not a toggle.",
    },
  },
  {
    id: "ollama_server",
  ...
  },
  {
    ...
    detail: {
      ...
      body: "On the model's stream ending cleanly: message.content is set to the full accumulated reply, status becomes \"complete\", sources (if RAG contributed) are attached, and it's all committed — then publish_done fires the final {done: true, sources, title} event. This is the ONLY path to status=\"complete\" — every other exit (an OllamaError, a timeout, a cancellation, an unhandled exception) instead leaves status=\"error\", by design, so a reply never just silently vanishes.",
    },
  },
  ...
  },
  {
    ...
    detail: {
      ...
      location: "app/services/ollama_client.py — embed, using app.config.EMBEDDING_MODEL",
      body: `${APP_NAME}'s own code builds and sends this request — a SEPARATE, smaller machine learning model from the conversation's own chat model, purpose-built for similarity search rather than text generation. The actual embedding computation doesn't happen here; it happens on Ollama, the next step.`,
    },
  },
  {
    id: "rag_embed_ollama",
  ...
  // ---- Branch: error / timeout / cancellation ------------------------
  {
    ...
    detail: {
      ...
      body: "An OllamaError or a reply-timeout both mark the message status=\"error\", persisting whatever partial content was generated so far — never silently discarded. A timeout ALSO force-unloads the model (ollama_client.stop_model): closing the HTTP connection to Ollama alone doesn't reliably stop an in-progress compute phase, especially prompt/image processing, which doesn't appear to check for client disconnection at all. A personal chat's sender-disconnect cancellation can't safely await anything in its own already-cancelled task, so it schedules a brand-new, uncancelled task to do this same persistence instead.",
    },
  },
...
const WORKFLOW_EDGES = [
  ...
  { from: "run_generation_task", to: "ollama_client_stream", branch: false },
  { from: "ollama_client_stream", to: "ollama_server", branch: false },
  { from: "ollama_server", to: "broadcast_publish", branch: false },
  ...
  { from: "rag_embed", to: "rag_embed_ollama", branch: true },
  { from: "rag_embed_ollama", to: "rag_rank", branch: true },
  ...
  { from: "ollama_client_stream", to: "termination_service", branch: true },
